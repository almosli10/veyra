...
export default function AddPlacePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { error } = await supabase.from('place_requests').insert({
      ...
      map_link: form.map_link, status: 'pending',
    })
      ...
      <div style={{ maxWidth: 680, margin: '0 auto' }}>
        ...
        <div className="glass neon-border" style={{ borderRadius: 28, padding: '36px 32px', animation: 'slideUp 0.6s 0.1s ease both', opacity: 0 }}>
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            {/* رابط Google Maps */}
            <div>
              <label className="form-label">📍 رابط الموقع على Google Maps</label>
              <input name="map_link" value={form.map_link} onChange={handleChange}
                placeholder="https://maps.app.goo.gl/..."
                className="form-input" />
              <p style={{ color: 'rgba(255,255,255,0.25)', fontSize: 11, marginTop: 6 }}>
                💡 افتح موقعك على Google Maps واضغط "مشاركة" ثم انسخ الرابط
              </p>
            </div>
            ...
            <button type="submit" disabled={loading || uploading}
              style={{ background: loading ? 'rgba(255,255,255,0.1)' : 'linear-gradient(135deg,#7C4DFF,#00E5FF)', color: loading ? 'rgba(255,255,255,0.4)' : 'white', border: 'none', borderRadius: 16, padding: '15px', fontSize: 15, fontWeight: 800, cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.3s ease', width: '100%', boxShadow: loading ? 'none' : '0 8px 24px rgba(124,77,255,0.3)' }}>
