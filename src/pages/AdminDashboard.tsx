...
interface PlaceRequest {
  id: number; name: string; name_ku: string; description: string
  address: string; phone: string; opening_hours: string; image: string
  status: string; created_at: string; latitude: number; longitude: number
  whatsapp: string; category_id: number; owner_id: string
}

interface Place {
  id: number; name: string; slug: string; address: string; image: string
  rating: number; reviews_count: number; featured: boolean; created_at: string
...
export default function AdminDashboard() {
  ...
  async function fetchRequests() {
    setLoading(true)
    const { data } = await supabase.from('place_requests').select('*').eq('status', tab).order('created_at', { ascending: false })
  ...
  async function fetchPlaces() {
    setLoading(true)
    const { data } = await supabase.from('places').select('*, categories(name, icon)').order('created_at', { ascending: false })
    if (data) {
      setPlaces(data)
      // جلب الصور لكل مكان
      const { data: imagesData } = await supabase.from('place_images').select('*')
      if (imagesData) {
        const imagesMap: {[key: number]: string[]} = {}
        imagesData.forEach((img: any) => {
          if (!imagesMap[img.place_id]) imagesMap[img.place_id] = []
          imagesMap[img.place_id].push(img.image_url)
        })
  ...
  async function handleUploadImages(placeId: number, files: FileList) {
    setUploadingFor(placeId)
    for (let i = 0; i < files.length; i++) {
      const file = files[i]
      const fileName = `${placeId}_${Date.now()}_${i}.${file.name.split('.').pop()}`
      ...
      if (!error && data) {
        ...
        await supabase.from('place_images').insert({ place_id: placeId, image_url: urlData.publicUrl })
      }
    }
  ...
  async function handleDeleteImage(placeId: number, imageUrl: string) {
    await supabase.from('place_images').delete().eq('place_id', placeId).eq('image_url', imageUrl)
    fetchPlaces()
  }
  ...
  async function handleApprove(req: PlaceRequest) {
    try {
      ...
      if (updateError) { alert(`تم نشر المكان لكن فشل تحديث الطلب: ${updateError.message}`); return }
  ...
  async function handleReject(id: number) {
    try {
      const { error } = await supabase.from('place_requests').update({ status: 'rejected' }).eq('id', id)
      if (error) { alert(`فشل رفض الطلب: ${error.message}`); return }
      alert('تم رفض الطلب ❌')
      fetchRequests()
    } catch (err) { alert('حدث خطأ غير متوقع') }
  }
  ...
  async function handleDeletePlace(id: number) {
    ...
    await supabase.from('place_images').delete().eq('place_id', id)
      ...
      <div style={{ background: '#080C1A', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(24px)', border: '1px solid rgba(124,77,255,0.3)', borderRadius: 28, padding: '48px 40px', maxWidth: 420, width: '100%', textAlign: 'center', animation: 'slideUp 0.6s ease', boxShadow: '0 0 60px rgba(124,77,255,0.15)' }}>
          ...
          <form onSubmit={handlePasswordSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <input type="password" value={passwordInput} onChange={e => setPasswordInput(e.target.value)} placeholder="كلمة المرور" className="form-input" required />
            {passwordError && <p style={{ color: '#f87171', fontSize: 12, margin: 0, background: 'rgba(239,68,68,0.1)', padding: '8px 12px', borderRadius: 10 }}>❌ {passwordError}</p>}
    ...
    <div style={{ background: '#080C1A', minHeight: '100vh', padding: '32px 16px' }}>
      ...
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 32 }}>
          <div>
            <h1 style={{ color: 'white', fontSize: 24, fontWeight: 900, margin: '0 0 6px', background: 'linear-gradient(90deg,#fff,#a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>لوحة الإدارة</h1>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 24, flexWrap: 'wrap' }}>
          {[
            ...
            <button key={t.key} onClick={() => setTab(t.key as any)} className="tab-btn"
              style={{ padding: '9px 18px', borderRadius: 12, fontSize: 13, fontWeight: 600, background: tab === t.key ? 'linear-gradient(135deg,#7C4DFF,#00E5FF)' : 'rgba(255,255,255,0.05)', color: tab === t.key ? 'white' : 'rgba(255,255,255,0.5)', border: tab === t.key ? 'none' : '1px solid rgba(255,255,255,0.08)', boxShadow: tab === t.key ? '0 4px 16px rgba(124,77,255,0.3)' : 'none' }}>
              {t.label}
            </button>
          ))}
        </div>
                ...
                <div key={req.id} className="glass card-item" style={{ borderRadius: 22, padding: '20px', animation: `slideUp 0.4s ${i*0.05}s ease both`, opacity: 0 }}>
                  <div style={{ display: 'flex', gap: 16 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                        ...
                        <span style={{ fontSize: 11, padding: '4px 12px', borderRadius: 999, fontWeight: 700, background: req.status === 'pending' ? 'rgba(251,191,36,0.15)' : req.status === 'approved' ? 'rgba(52,211,153,0.15)' : 'rgba(239,68,68,0.15)', color: req.status === 'pending' ? '#fbbf24' : req.status === 'approved' ? '#34d399' : '#f87171' }}>
                  ...
                  {tab === 'pending' && (
                    <div style={{ display: 'flex', gap: 10, marginTop: 16, paddingTop: 16, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                      <button onClick={() => handleApprove(req)} style={{ flex: 1, background: 'linear-gradient(135deg,#10b981,#059669)', color: 'white', border: 'none', borderRadius: 12, padding: '11px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>✅ قبول ونشر</button>
                      <button onClick={() => handleReject(req.id)} style={{ flex: 1, background: 'rgba(239,68,68,0.1)', color: '#f87171', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 12, padding: '11px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>❌ رفض</button>
                    </div>
                  )}
                </div>
              ))}
        ...
        {tab === 'places' && (
          loading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              {places.map((place, i) => (
                <div key={place.id} className="glass card-item" style={{ borderRadius: 18, padding: '14px 18px', animation: `slideUp 0.4s ${i*0.04}s ease both`, opacity: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                        ...
                        {placeImages[place.id]?.length > 0 && <span style={{ color: '#00E5FF', fontSize: 11 }}>🖼 {placeImages[place.id].length} صور إضافية</span>}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                      {/* زر رفع صور */}
                      <label title="إضافة صور" style={{ background: 'rgba(0,229,255,0.1)', border: '1px solid rgba(0,229,255,0.3)', borderRadius: 10, padding: '7px 12px', fontSize: 14, cursor: 'pointer', color: '#00E5FF', display: 'flex', alignItems: 'center' }}>
                        {uploadingFor === place.id ? '⏳' : '🖼'}
                        <input type="file" accept="image/*" multiple onChange={e => e.target.files && handleUploadImages(place.id, e.target.files)} style={{ display: 'none' }} disabled={uploadingFor === place.id} />
                      </label>
                      {/* زر تمييز */}
                      <button onClick={() => toggleFeatured(place.id, place.featured)} title={place.featured ? 'إلغاء التمييز' : 'تمييز المكان'}
                        style={{ background: place.featured ? 'rgba(124,77,255,0.3)' : 'rgba(255,255,255,0.06)', border: place.featured ? '1px solid rgba(124,77,255,0.5)' : '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '7px 12px', fontSize: 14, cursor: 'pointer', color: place.featured ? '#a78bfa' : 'rgba(255,255,255,0.4)' }}>
                        ⭐
                      </button>
                      {/* زر حذف */}
                      {deleteConfirm === place.id ? (
                        <div style={{ display: 'flex', gap: 6 }}>
                          <button onClick={() => handleDeletePlace(place.id)} style={{ background: 'rgba(239,68,68,0.9)', color: 'white', border: 'none', borderRadius: 10, padding: '7px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>تأكيد</button>
                          <button onClick={() => setDeleteConfirm(null)} style={{ background: 'rgba(255,255,255,0.06)', color: 'rgba(255,255,255,0.5)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '7px 12px', fontSize: 12, cursor: 'pointer' }}>إلغاء</button>
                        </div>
                      ) : (
                        <button onClick={() => setDeleteConfirm(place.id)} style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#f87171', borderRadius: 10, padding: '7px 12px', fontSize: 14, cursor: 'pointer' }}
                  ...
                  {placeImages[place.id]?.length > 0 && (
                    <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid rgba(255,255,255,0.06)', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                      {placeImages[place.id].map((img, idx) => (
                        <div key={idx} style={{ position: 'relative' }}>
                          <img src={img} alt="" style={{ width: 60, height: 60, objectFit: 'cover', borderRadius: 10 }} />
                          <button onClick={() => handleDeleteImage(place.id, img)}
                            style={{ position: 'absolute', top: -6, right: -6, background: 'rgba(239,68,68,0.9)', color: 'white', border: 'none', borderRadius: '50%', width: 20, height: 20, fontSize: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            ✕
                          </button>
                        </div>
                      ))}
